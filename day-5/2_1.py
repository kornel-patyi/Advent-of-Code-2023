...
def make_children_no_return(start_range: SplitRange, map_index) -> list:
    map_category = maps[map_index]
    children_ref = start_range
    for curr_map in map_category:
        if curr_map.source + curr_map.length <= start_range.destination or curr_map.source >= start_range.destination + start_range.length:
            continue
        future_start = max(curr_map.source, start_range.destination)
        future_end = min(curr_map.source + curr_map.length, start_range.destination + start_range.length)
        future_destination = future_start - (curr_map.source - curr_map.destination)
        children_ref.children.append(SplitRange(future_destination, future_start, future_end - future_start, []))

    if len(children_ref.children) == 0:
        children_ref.children = [SplitRange(start_range.destination, start_range.destination, start_range.length, [])]

    children_len_sum = 0
    for child in children_ref.children:
        children_len_sum += child.length
    if start_range.length == children_len_sum:
        return None
    ...
    for child in children_ref.children:
        new_start_range = new_start_range.subtract(child)
    if type(new_start_range) is CalcRange:
        children_ref.children.append(SplitRange(new_start_range.source, new_start_range.source, new_start_range.length, []))
        return None
    for uncovered_range in new_start_range.ranges:
        children_ref.children.append(SplitRange(uncovered_range.source, uncovered_range.source, uncovered_range.length, []))
